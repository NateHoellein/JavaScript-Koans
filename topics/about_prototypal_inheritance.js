...
// things that don't need to be set in the constructor should be added to the constructor's prototype property.
  ...
  it("defining a 'class'", function() {
      ...
      expect(eric.sayHi()).toBe("Hello, my name is Eric"); // 'what will Eric say?');
  });
...
Mammal.prototype.favouriteSaying = function() {
  ...
  it("more functions", function() {
      ...
      expect(bobby.favouriteSaying()).toBe("Bobby's favourite saying is Hello, my name is Bobby"); // "what is Bobby's favourite saying?"); 
  });
  ...
  it("calling functions added to a prototype after an object was created", function() {
      ...
      expect(paul.numberOfLettersInName()).toBe(4); // "how long is Paul's name?");
  });
...
extend(Bat, Mammal);
  ...
  it("Inheritance", function() {
      ...
      expect(lenny.sayHi()).toBe("Hello, my name is Lenny"); // "what does Lenny say?");
      expect(lenny.wingspan).toBe("1.5m"); // "what is Lenny's wingspan?");
  });
});
/*
